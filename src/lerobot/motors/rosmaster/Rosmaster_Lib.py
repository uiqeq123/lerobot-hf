...
class Rosmaster(object):
    ...
    def __arm_convert_value(self, s_id, s_angle):
        ...
        if s_id == 1:
            ...
            # 角度-360°对应脉冲900，角度360°对应脉冲3100
            value = int((3100 - 900) * (s_angle - (-360)) / (360 - (-360)) + 900)
        elif s_id == 4:
            value = int((3100 - 900) * (s_angle - (-360)) / (360 - (-360)) + 900)
        elif s_id == 5:
    ...
    def __arm_convert_angle(self, s_id, s_value):
        ...
        if s_id == 1:
            ...
            # 脉冲900对应-360°，脉冲3100对应360°
            s_angle = int((s_value - 900) * (360 - (-360)) / (3100 - 900) + (-360) + 0.5)
        elif s_id == 4:
            s_angle = int((s_value - 900) * (360 - (-360)) / (3100 - 900) + (-360) + 0.5)
        elif s_id == 5:
    ...
    def set_car_run(self, state, speed, adjust=False):
        try:
            ...
            # 检查是否有错误响应
            # 这里可以添加检查底层返回的错误信息的逻辑
            # 如果检测到移动错误，抛出异常
            # if self.__car_move_error:
            #     raise Exception("CAR MOVE ERROR")
        except Exception as e:
            # 如果原始错误信息中没有明确说明是移动错误，则添加我们自己的错误信息
            error_msg = str(e)
            if "car move" not in error_msg.lower():
                error_msg = "CAR MOVE ERROR: " + error_msg
    ...
    def set_uart_servo_angle_array(self, angle_s=[90, 90, 90, 90, 90, 180], run_time=500):
        try:
            if not self.__arm_ctrl_enable:
                return
            if -46 <= angle_s[0] <= 180 and -180 <= angle_s[1] <= 180 and -360 <= angle_s[2] <= 360 and \
                    -360 <= angle_s[3] <= 360 and 0 <= angle_s[4] <= 270 and 0 <= angle_s[5] <= 180:
                return
            if -46 <= angle_s[0] <= 180 and -25 <= angle_s[1] <= 180 and -360 <= angle_s[2] <= 360 and \
                    -360 <= angle_s[3] <= 360 and 0 <= angle_s[4] <= 270 and 0 <= angle_s[5] <= 180:
                return
            if -46 <= angle_s[0] <= 180 and -25 <= angle_s[1] <= 180 and -360 <= angle_s[2] <= 360 and \
                    -360 <= angle_s[3] <= 360 and 0 <= angle_s[4] <= 270 and 0 <= angle_s[5] <= 180:
            ...
            if -46 <= angle_s[0] <= 180 and -25 <= angle_s[1] <= 180 and -360 <= angle_s[2] <= 360 and \
                ...
                print("angle_s input error!:", angle_s)
        ...
        try:
            ...
            while timeout > 0:
                if self.__read_arm_ok == 1:
                    for i in range(6):
                        if self.__read_arm[i] > -361:
                            converted_angle = self.__arm_convert_angle(i + 1, self.__read_arm[i])
                            # Validate angle ranges for each joint
                            if i + 1 == 1:  # Joint 1: -180 to 180
                                if -180 <= converted_angle <= 180:
                                    angle[i] = converted_angle
                            elif i + 1 == 2:  # Joint 2: -180 to 180
                                if -180 <= converted_angle <= 180:
                                    angle[i] = converted_angle
                            elif i + 1 == 3:  # Joint 3: -360 to 360
                                if -360 <= converted_angle <= 360:
                                    angle[i] = converted_angle
                            elif i + 1 == 4:  # Joint 4: -360 to 360
                                if -360 <= converted_angle <= 360:
                                    angle[i] = converted_angle
                            elif i + 1 == 5:  # Joint 5: 0 to 270
                                if 0 <= converted_angle <= 270:
                                    angle[i] = converted_angle
                            elif i + 1 == 6:  # Joint 6: 0 to 180
                                if 0 <= converted_angle <= 180:
                                    angle[i] = converted_angle
                    if self.__debug:
